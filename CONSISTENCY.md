...
in `landing-page`; the identical document is mirrored across the family.
Changes are made family-wide in one sweep or not at all.

Authority: the effort map at `.scratch/nanisoft-web/map.md` (in the Nanisoft
workspace folder, sibling of these repos) and its context doc `CONTEXT.md`
are the shared decisions ledger — a session in any repo starts there. Ticket
numbers below refer to that map.
...
- Migrating onto the new Prism line is a redesign of every site — a new
  effort with its own map, not maintenance on this family.

## 2. Shared chrome is npm-only

`SiteHeader`/`SiteFooter`/`PrismThemeModeProvider`/the boot script come from
prism-ui — never copied into an app. Import shapes (law from scaffold ticket
05): components via per-component subpaths, blocks via `/blocks`, provider
via `/provider`, the boot script via the root barrel; never
`@nanisoft/prism-ui/theming` in app code (it pulls the SSR extractor). antd
ships `'use client'`: `Typography.*` and `DisplayTitle` cross the RSC
boundary only through the site's `components/prism-client.tsx`.
...
## 4. Canvas re-theme law (tickets 11, 17)
...
pixel-sampling, ticket 11; found live in AlphaLens, fixed in ticket 17).

## 5. Reveal-safety law (tickets 10–13)
...
## 6. Docs/blog shell styles are parity-locked (ticket 16)
...
## 7. Content tests read from disk (ticket 10)
...
| Repo | Copy |
| --- | --- |
| landing-page | `CONSISTENCY.md` (this file) |
| nexus | `CONSISTENCY.md` |
| atlas | `CONSISTENCY.md` |
| alphalens | `CONSISTENCY.md` |
| prism | pending — the rebuilt prism repo was hands-off for this effort; land it when the prism effort resumes |

*Contract version 1.0, 2026-09-26. No sweeps have been required (frozen
line).*
